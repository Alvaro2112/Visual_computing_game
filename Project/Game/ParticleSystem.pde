...
class ParticleSystem {
  ...
  boolean checkPosition(PVector center) {
    ...
    if (center.x <= -xBox/2 || center.x >= xBox/2   || center.z <= -xBox/2  || center.z >= xBox/2 ) { //Add particle radius? how to know what is the radius? add a parameter?
      return false;
    }
    return true;
  }
