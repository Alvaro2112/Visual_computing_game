...
void draw() {
  if (!done) {
    PImage quads = thresholdTool.thresholdHueWithOriginalColor(img, 100, 138, 100, MAX_COLOR, MIN_COLOR, 200);
    quads = thresholdTool.thresholdHue(quads, 100, 150, 70, MAX_COLOR, MIN_COLOR, 200);
    ...
    PVector rotations = twoDthreeD.get3DRotations(homogeneousCorners);
    println("rx: " + (rotations.x*180.0/PI - 180) + ", ry: " + rotations.y*180.0/PI + ", rz: " + rotations.z*180.0/PI);
    // 12.913467, ry: -42.962616, rz: -4.462464 -> board 4
    // rx: -9.575562, ry: 14.022539, rz: 3.3341608 -> board 1
    done = true;
  }
