class Sphere {
  ...
  void checkCylinders() {
    for (closedCylinder c : Cylinders) {
      PVector n = new PVector(c.location.x - location.x, 0,c.location.z - location.z);
      if (sqrt(n.x*n.x + n.z*n.z) <= closedCylinder.cylinderBaseSize + RADIUS){
